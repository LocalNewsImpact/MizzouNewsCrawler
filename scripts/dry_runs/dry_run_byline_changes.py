...
def analyze_byline_changes():
    ...
    for (
        ...
        try:
            ...
            new_authors = cleaner.clean_byline(raw_byline, source_name=source_name)

            # Compare results
            current_norm = sorted([a.strip() for a in current_authors if a.strip()])
            new_norm = sorted([a.strip() for a in new_authors if a.strip()])
            ...
            if current_norm == new_norm:
                ...
                if new_count > current_count:
                    ...
                    has_html_fix = any("&#" in author for author in current_norm)
                    if has_html_fix:
    ...
    if stats["articles_with_raw_byline"] > 0:
        ...
        print("-" * 40)
        unchanged_pct = (
            stats["unchanged_bylines"] / stats["articles_with_raw_byline"]
        ) * 100
        changed_pct = (
            stats["changed_bylines"] / stats["articles_with_raw_byline"]
        ) * 100
        ...
        if stats["changed_bylines"] > 0:
            ...
            print(
                f"Improvements: {stats['improved_bylines']} ({improved_pct:.1f}% of changes)"
            )
            print(
                f"Degradations: {stats['degraded_bylines']} ({degraded_pct:.1f}% of changes)"
            )
            print()
    ...
    if stats["articles_with_raw_byline"] == 0:
        ...
        print(
            f"Change impact: {stats['changed_bylines']}/{stats['articles_with_raw_byline']} articles would change"
        )
