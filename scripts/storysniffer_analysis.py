...
def categorize_non_articles(non_articles):
    ...
    for item in non_articles:
        ...
        if any(
            pattern in url for pattern in ["/search", "/tag", "/category", "/author"]
        ):
            ...
            pattern in url for pattern in ["/contact", "/about", "/privacy", "/terms"]
        ):
            ...
            pattern in url
            for pattern in [".jpg", ".png", ".gif", ".pdf", ".css", ".js"]
        ):
...
def main():
    ...
    if not STORYSNIFFER_AVAILABLE:
        logger.error(
            "StorySniffer is not available. Please install with: pip install storysniffer"
        )
        return 1
    ...
    # Track results
    results = {"articles": [], "non_articles": [], "errors": []}

    source_stats = defaultdict(
        lambda: {"total": 0, "articles": 0, "non_articles": 0, "errors": 0}
    )
    ...
    print(
        f"  Likely articles: {article_count} ({article_count / total_analyzed * 100:.1f}%)"
    )
    print(
        f"  Non-articles: {non_article_count} ({non_article_count / total_analyzed * 100:.1f}%)"
    )
    ...
    print("\nSource-by-Source Breakdown:")
    print(
        f"{'Source':<30} {'Total':<8} {'Articles':<10} {'Non-Articles':<12} {'Article %':<10}"
    )
    print("-" * 75)

    for source, stats in sorted(source_stats.items()):
        article_pct = (
            (stats["articles"] / stats["total"] * 100) if stats["total"] > 0 else 0
        )
        print(
            f"{source:<30} {stats['total']:<8} {stats['articles']:<10} {stats['non_articles']:<12} {article_pct:<10.1f}%"
        )
    ...
    if results["errors"]:
        print("\nError Analysis:")
        error_types = Counter(
            item.get("error", "Unknown") for item in results["errors"]
        )
