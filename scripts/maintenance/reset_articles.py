...
def reset_last_articles(count=15):
    ...
    try:
            ...
            articles_to_remove.append(
                {
                    ...
                    "url": row.url,
                    "title": row.title[:50] + "..."
                    if row.title and len(row.title) > 50
                    else row.title,
                    "author": row.author,
        ...
        if candidate_link_ids:
            logger.info(
                f"Resetting {len(candidate_link_ids)} candidate_links status to 'article'..."
            )

            # Create placeholders for the IN clause
            cl_placeholders = ",".join(
                [":cl_id" + str(i) for i in range(len(candidate_link_ids))]
            )
            cl_params = {
                f"cl_id{i}": cl_id for i, cl_id in enumerate(candidate_link_ids)
            }
        ...
        logger.info(
            f"Removed {len(article_ids)} articles and reset {len(candidate_link_ids)} candidate_links."
        )
        logger.info(
            "These articles can now be re-extracted with the fixed byline cleaning."
        )
