...
from models.database import (
    DatabaseManager, upsert_candidate_link, create_job_record, 
    finish_job_record, export_to_parquet
)
...
def crawl_site(crawler: NewsCrawler, site_config: Dict[str, any], 
               db_manager: DatabaseManager, job_id: str) -> Dict[str, int]:
...
def main():
    ...
    parser.add_argument('--sources', required=True,
                       help='JSON file with site configurations')
    parser.add_argument('--output-db', default='data/mizzou.db',
        ...
        with DatabaseManager(f"sqlite:///{args.output_db}") as db_manager:
            ...
            for site_config in sources:
                try:
                    ...
                    total_metrics['total_links_discovered'] += site_metrics['links_discovered']
                    total_metrics['total_links_saved'] += site_metrics['links_saved']
                    total_metrics['total_errors'] += site_metrics['errors']
                    
                except Exception as e:
                    logger.error(f"Error processing site {site_config.get('name', 'unknown')}: {e}")
            ...
            finish_job_record(
                db_manager.session,
                job.id,
                exit_status='success' if total_metrics['total_errors'] == 0 else 'partial_success',
                metrics={
            ...
            if args.export_snapshot:
                snapshot_path = Path(args.artifacts_dir) / f"candidate_links_{args.job_id}.parquet"
                export_to_parquet(
                    db_manager.engine,
                    'candidate_links',
                    str(snapshot_path),
