...
def reset_extracted_articles():
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM candidate_links WHERE status = 'extracted'"
        )
        ...
        # Reset status
        cursor.execute(
            "UPDATE candidate_links SET status = 'article' WHERE status = 'extracted'"
        )
        ...
        print(
            f"Successfully reset {rows_affected} articles from 'extracted' to 'article' status"
        )
        ...
        print(
            f"Verification: {remaining_extracted} extracted, {total_articles} articles"
        )
