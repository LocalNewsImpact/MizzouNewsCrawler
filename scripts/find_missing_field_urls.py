...
def find_urls_with_missing_fields(limit=10, days_back=30):
    ...
    try:
        ...
        print(
            f"(Looking back {days_back} days from {cutoff_date.strftime('%Y-%m-%d')})"
        )
        print(f"{'=' * 80}")
...
def find_specific_patterns():
    ...
    patterns = [
        (
            "Missing only author",
            "title IS NOT NULL AND title != '' AND content IS NOT NULL AND LENGTH(content) >= 100 AND publish_date IS NOT NULL AND publish_date != '' AND (author IS NULL OR author = '')",
        ),
        (
            "Missing only title",
            "author IS NOT NULL AND author != '' AND content IS NOT NULL AND LENGTH(content) >= 100 AND publish_date IS NOT NULL AND publish_date != '' AND (title IS NULL OR title = '')",
        ),
        (
            "Missing only date",
            "title IS NOT NULL AND title != '' AND author IS NOT NULL AND author != '' AND content IS NOT NULL AND LENGTH(content) >= 100 AND (publish_date IS NULL OR publish_date = '')",
        ),
        (
            "Missing only content",
            "title IS NOT NULL AND title != '' AND author IS NOT NULL AND author != '' AND publish_date IS NOT NULL AND publish_date != '' AND (content IS NULL OR LENGTH(content) < 100)",
        ),
        (
            "Missing multiple fields",
            "(title IS NULL OR title = '') AND (author IS NULL OR author = '') AND (content IS NULL OR LENGTH(content) < 100)",
        ),
    ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find URLs with missing fields for testing"
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of URLs to return (default: 10)"
    )
    parser.add_argument(
        "--days", type=int, default=30, help="Days back to search (default: 30)"
    )
    parser.add_argument(
        "--patterns", action="store_true", help="Show specific missing field patterns"
    )
    ...
    if args.patterns:
        ...
        if urls:
            ...
            print(
                "You can copy these URLs into your test methods to replace the mock data."
            )
        else:
            print("\nNo URLs with missing fields found in the specified time range.")
            print(
                "Try increasing --days or check if extractions have been running recently."
            )
