...
def compute_text_hash(text: str) -> str:
    """Compute hash of article text, at the length the corpus uses.

    Truncating to 32 characters made these rows undetectable as duplicates of
    anything the crawler wrote, which all carry the full 64-character digest.
    """
...
def import_articles(file_path: str, dry_run: bool = False, batch_size: int = 100):
    ...
    with db.get_session() as session:
        for record in records:
            ...
            exists = session.execute(
                text("SELECT 1 FROM candidate_links WHERE url = :url"), {"url": url}
            ).fetchone()
    ...
    print(
        f"     curl -H 'Authorization: Bearer $(gcloud auth print-identity-token)' \\"
    )
    print(
        f"       'https://us-central1-mizzou-news-crawler.cloudfunctions.net/daily-sheet-exporter?date=YYYY-MM-DD'"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Import manual articles to production DB"
    )
