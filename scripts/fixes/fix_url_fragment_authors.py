...
def find_articles_with_url_fragments(db_path: str) -> List[Tuple]:
    ...
    log_info(f"Found {len(problematic_articles)} articles with URL fragments in authors")
    return problematic_articles


def fix_article_authors(article_data: List[Tuple], db_path: str, dry_run: bool = True) -> Dict:
    """
    ...
    try:
        for article_id, url, title, current_authors, url_fragments in article_data:
            ...
            clean_authors = [
                author for author in current_authors 
...
def main():
    ...
    parser.add_argument('--article-id', type=str,
                        help='Fix specific article ID only')
    ...
    if args.article_id:
        problematic_articles = [
            article for article in problematic_articles
            if article[0] == args.article_id
        ]
        
        if not problematic_articles:
            print(f"❌ Article {args.article_id} not found or doesn't have URL fragment issues")
            return
    ...
    for i, (article_id, url, title, authors, fragments) in enumerate(problematic_articles, 1):
