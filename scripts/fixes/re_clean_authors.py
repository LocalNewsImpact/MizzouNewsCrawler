...
def re_clean_authors(
    telemetry_data: list[tuple], db_path: str, dry_run: bool = True
) -> dict:
    """
    ...
    for (
        ...
        try:
            if current_author:
                current_authors = (
                    json.loads(current_author)
                    if isinstance(current_author, str)
                    else current_author
                )
        ...
        current_set = set(
            author.lower().strip() for author in current_authors if author
        )
        ...
        if current_count == 0 and new_count > 0:
            ...
            for new_author in new_authors:
                if "'" in new_author:  # Has apostrophe
                    ...
                    for current_author in current_authors:
                        if (
                            new_author.replace("'", "").lower()
                            in current_author.lower()
                        ):
                            improvements_found.append(
                                f"Fixed apostrophe: '{current_author}' -> '{new_author}'"
                            )
            ...
            for new_author in new_authors:
                if len(new_author.split()) == 2:  # "First Last" format
                    ...
                    for current_author in current_authors:
                        if (
                            current_author.lower().strip()
                            == f"{last.lower()}, {first.lower()}"
                        ):
                            improvements_found.append(
                                f"Fixed name order: '{current_author}' -> '{new_author}'"
                            )
                ...
                if any(word in current_author.lower() for word in title_words):
                    for new_author in new_authors:
                        if not any(word in new_author.lower() for word in title_words):
                            if new_author.lower().replace(
                                " ", ""
                            ) in current_author.lower().replace(" ", ""):
                                improvements_found.append(
                                    f"Removed title: '{current_author}' -> '{new_author}'"
                                )
        ...
        if not dry_run and is_improvement and cursor:
            ...
            log_info(
                f"Updated article {article_id}: {current_authors} -> {new_authors}"
            )
...
def print_re_cleaning_report(stats: dict):
    ...
    if stats["total_processed"] > 0:
        improvement_rate = (
            (stats["improved"] + stats["empty_to_filled"])
...
def main():
    ...
    print("\n" + "=" * 60)
    proceed = input(
        "\n🤔 Do you want to apply these improvements to the database? (y/N): "
    ).lower()
    ...
    if proceed == "y":
        ...
        print("\n✅ Database updated!")
        print(
            f"   {real_stats['improved'] + real_stats['empty_to_filled']} articles improved"
        )
