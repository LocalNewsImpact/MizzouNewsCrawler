...
def show_removal_text():
    ...
    parser.add_argument(
        "--sample-size", type=int, default=15, help="Number of articles to sample"
    )
    parser.add_argument(
    ...
    print(
        f"Sample: {args.sample_size} articles, Min occurrences: {args.min_occurrences}"
    )
    ...
    for i, segment in enumerate(results["segments"], 1):
        ...
        print(
            f"Length: {segment['length']} chars | Occurrences: {segment['occurrences']} articles"
        )
    ...
    print(
        f"Average segment length: {sum(s['length'] for s in results['segments']) // len(results['segments'])} chars"
    )
