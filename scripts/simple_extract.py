...
def update_candidate_status(
    db, candidate_id, status, publish_date=None, error_message=None
):
...
def create_article_record(db, candidate, extraction_result):
    ...
    article_data = {
        ...
        "word_count": len(extraction_result.get("content", "").split())
...
def main():
    ...
    for i, candidate in enumerate(candidates, 1):
        ...
        try:
            ...
            # Update candidate status to 'failed'
            update_candidate_status(db, candidate["id"], "failed", error_message=str(e))
