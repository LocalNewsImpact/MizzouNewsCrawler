            ...
            with db.engine.begin() as conn:
                ...
                if "discovered_sections" in columns_to_add:
                    if dialect == "postgresql":
                        conn.execute(
                            text(
                                "ALTER TABLE sources ADD COLUMN discovered_sections JSONB"
                            )
                        )
                    else:
                        conn.execute(
                            text(
                                "ALTER TABLE sources ADD COLUMN discovered_sections TEXT"
                            )
                        )
                ...
                if "section_last_updated" in columns_to_add:
                    conn.execute(
                        text(
                            "ALTER TABLE sources ADD COLUMN section_last_updated TIMESTAMP"
                        )
                    )
