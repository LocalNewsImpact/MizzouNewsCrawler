...
@pytest.mark.skipif(not POSTGRES_DSN, reason="Postgres DSN not provided in POSTGRES_TEST_DSN")
def test_export_snapshot_postgres(tmp_path):
    ...
    dv2 = export_snapshot_for_version(dv.id, table_name, str(out), database_url=POSTGRES_DSN)
