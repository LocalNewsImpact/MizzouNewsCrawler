...
def test_parse_perflog_collects_px_tokens_in_multiple_fields():
        ...
        make_entry(
            "Network.requestWillBeSent",
            {
                ...
                "request": {
                    ...
                    "headers": {"Cookie": "_px=ABC123; other=1"},
                },
                "type": "Document",
                "timestamp": 1.0,
            },
    ...
    entries.append(
        make_entry(
            ...
            {"requestId": "r1", "info": "contains _px=IN-MEME"},
        )
    )
    ...
    r = rows[0]
    assert (
        "_px" in r["px_tokens"] or "pxhd" in r["px_tokens"] or "_px" in r["px_tokens"]
    )
...
def test_parse_perflog_domain_filtering():
    ...
    entries.append(
        make_entry(
            ...
            {
                "requestId": "r1",
                "request": {"url": "https://images.example.com/x.jpg"},
                "timestamp": 1.0,
            },
        )
    )
...
def test_parse_perflog_large_synthetic_runs_fast_and_consistent():
    ...
    for i in range(500):
        entries.append(
            make_entry(
                "Network.requestWillBeSent",
                {
                    "requestId": f"r{i}",
                    "request": {
                        "url": f"https://host{i % 5}.example.com/asset{i}.js",
        ...
        entries.append(
            make_entry(
                "Network.responseReceived",
                {"requestId": f"r{i}", "response": {"status": 200}},
            )
        )
