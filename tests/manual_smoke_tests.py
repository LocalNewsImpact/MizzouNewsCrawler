...
def test_bot_protection_detection():
    ...
    response.text = (
        "<html><body><h1>Access Denied</h1><p>Security check required</p></body></html>"
    )
    protection = extractor._detect_bot_protection_in_response(response)
    tests_total += 1
    ...
    response.text = (
        "<html><head><title>News Article</title></head><body><article>"
        + "<p>Content paragraph</p>" * 50
        + "</article></body></html>"
    )
...
def test_real_domain_smoke():
    ...
    try:
        ...
        if status == "success":
            ...
            print(
                f"   ℹ️  Extraction didn't succeed, but not bot-blocked (status: {status})"
            )
...
def test_user_agent_pool():
    ...
    has_modern_chrome = any(
        f"Chrome/{v}" in ua
        for ua in extractor.user_agent_pool
        for v in ["127", "128", "129"]
    )
    has_modern_firefox = any(
        f"Firefox/{v}" in ua for ua in extractor.user_agent_pool for v in ["130", "131"]
    )
    ...
    has_bot_strings = any(
        "bot" in ua.lower() or "crawler" in ua.lower()
        for ua in extractor.user_agent_pool
    )
