...
class TestMigrationWorkflow:
    ...
    def test_migration_version_tracking(self, tmp_path):
        ...
        with engine.connect() as conn:
            ...
            assert all(
                c in "0123456789abcdef" for c in version.lower()[:12]
    ...
    def test_rollback_and_reapply_migration(self, tmp_path):
        ...
        if version_after_downgrade == version_after_upgrade:
            ...
            pytest.skip(
                "Downgrade was a no-op in this migration history (merge revision present)"
            )
        ...
        assert (
            version_after_reupgrade == version_after_upgrade
