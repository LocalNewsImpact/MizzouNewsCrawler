...
def detect_og_distributor(metadata: dict) -> dict | None:
    ...
    distributor = metadata.get("og_distributor_category") or metadata.get(
...
def detect_jsonld_syndication(metadata: dict) -> dict | None:
    ...
    for item in items:
        ...
        if is_based_on:
            url = (
                is_based_on
                if isinstance(is_based_on, str)
                else is_based_on.get("url", "")
            )
...
def run_new_detection(article: dict, patterns: dict) -> dict | None:
    ...
    detectors = [
        (
            ...
            lambda: detect_canonical_cross_domain(metadata, url),
        ),
...
def main():
    ...
    with db.get_session() as session:
        ...
        print(
            f"Loaded patterns: {len(patterns['url'])} URL, {len(patterns['author'])} author, {len(patterns['content'])} content",
            file=sys.stderr,
        )
        ...
        print(
            f"\nResults: {detected_count}/{len(articles)} labeled articles would be detected as wire",
            file=sys.stderr,
        )
        print(
            f"Detection rate: {100*detected_count/len(articles):.2f}%", file=sys.stderr
        )
