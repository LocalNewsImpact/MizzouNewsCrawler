...
for url in urls_to_test:
    ...
    try:
        response = requests.get(
            url, proxies=proxies, headers=headers, timeout=15, allow_redirects=True
        )
        ...
        if response.status_code == 200:
            ...
            if "access denied" in content_lower or "blocked" in content_lower:
                print("   ⚠️  200 OK but content shows blocking message")
                print(f"   Preview: {response.text[:200]}")
            elif "<!doctype html" in content_lower or "<html" in content_lower:
                print("   ✅ Valid HTML page received")
                # Check for article indicators
                if "article" in content_lower or "news" in content_lower:
                    print("   ✅ Contains news/article content")
            elif "<?xml" in response.text[:100]:
                print("   ✅ Valid XML feed/sitemap")
            else:
                print(f"   Preview: {response.text[:150]}")

        elif response.status_code == 403:
            print("   ❌ 403 FORBIDDEN - Still blocked even with residential proxy")
            print(f"   Headers: {dict(response.headers)}")

        elif response.status_code >= 500:
            print("   ⚠️  Server error (not a bot block)")
