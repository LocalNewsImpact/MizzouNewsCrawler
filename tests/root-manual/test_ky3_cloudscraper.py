...
class KY3CloudScraperTester:
    ...
    def discover_urls(self, limit: int = 5) -> list[str]:
        ...
        try:
            ...
            logger.info("📡 Fetching homepage with CloudScraper...")
    ...
    def extract_article(self, url: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.info("   Step 1: CloudScraper fetch...")
            response = self.scraper.get(
                url,
                timeout=10,
                headers={
            ...
            logger.info("   Step 2: Newspaper4k extraction...")
            article = Article(url)
    ...
    def run_full_test(self, discovery_limit: int = 3, extract_limit: int = 2) -> dict:
        ...
        summary = {
            ...
            "discovery": {"attempted": True, "urls_found": 0, "urls": []},
            "extraction": {
                "attempted": 0,
                "successful": 0,
                "failed": 0,
...
def main():
    ...
    parser.add_argument(
        "--json-output", type=str, help="Output test results to JSON file"
    )
    ...
    results = tester.run_full_test(
        discovery_limit=args.discovery_limit, extract_limit=args.extract_limit
    )
    ...
    if (
        results["extraction"]["successful"] > 0
        or results["discovery"]["urls_found"] > 0
    ):
        return 0
    else:
        return 1
