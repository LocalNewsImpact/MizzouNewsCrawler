...
def mock_extractor_class(mock_entity_extractor):
    ...
    with patch(
        'src.pipeline.entity_extraction.ArticleEntityExtractor'
    ) as mock_class:
...
def test_batch_size_default_is_500():
    ...
    if 'GAZETTEER_BATCH_SIZE' not in os.environ:
        ...
        assert processor_module.GAZETTEER_BATCH_SIZE == expected_default or \
               os.getenv("GAZETTEER_BATCH_SIZE") is not None
