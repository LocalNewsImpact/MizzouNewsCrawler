...
class TestPerimeterXBypass:
    def test_detect_captcha_or_challenge_perimeterx(self):
        ...
        mock_driver.find_elements.side_effect = lambda by, sel: [Mock()] if sel == "#px-captcha" else []
        assert extractor._detect_captcha_or_challenge(mock_driver) is True
    ...
    def test_navigate_order_challenge_before_subscription(self):
        ...
        with patch("src.crawler.WebDriverWait"), \
            ...
            with patch.object(extractor, "_detect_captcha_or_challenge") as mock_detect_challenge, \
                ...
                extractor._navigate_with_human_behavior(mock_driver, "https://example.com")
                ...
                relevant_calls = [name for name in call_names if name in ('detect_challenge', 'close_modals', 'detect_sub')]
    ...
    def test_try_bypass_challenge_perimeterx_long_press(self):
        ...
        with patch("selenium.webdriver.common.action_chains.ActionChains") as mock_actions:
            ...
            with patch.object(extractor, "_detect_captcha_or_challenge", side_effect=[True, False]):
    ...
    def test_mark_domain_special_extraction_perimeterx(self):
        ...
        with patch("src.models.database.DatabaseManager") as mock_db_mgr:
            ...
            mock_db_mgr.return_value.get_session.return_value.__enter__.return_value = mock_session
