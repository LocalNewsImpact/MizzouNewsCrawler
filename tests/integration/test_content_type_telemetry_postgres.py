...
@pytest.fixture
def test_operation(cloud_sql_session):
    """Create a test operation for telemetry."""
    operation = Operation(
        id=str(uuid.uuid4()),
    ...
    cloud_sql_session.commit()
    cloud_sql_session.refresh(operation)
    return operation


def test_content_type_telemetry_with_string_confidence(
    cloud_sql_session, test_article, test_operation
):
