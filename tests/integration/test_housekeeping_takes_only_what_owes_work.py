...
def db():
    ...
    with factory() as s:
        s.execute(
            sa.text(
                "DELETE FROM pipeline_rework WHERE requested_by='test' "
                "OR record_id LIKE 'hk-%'"
            )
        )
        s.execute(
    ...
    with db() as s:
        ...
        s.execute(
            sa.text(
                ...
                "WHEN 'hk-link-0' THEN 'extracted' WHEN 'hk-link-1' THEN 'paused' "
                "ELSE status END WHERE id LIKE 'hk-link-%'"
            )
        )
        s.commit()
