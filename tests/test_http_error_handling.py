...
class TestNotFoundErrorHandling:
    """Test 404/410 responses stop fallback attempts (commit 615f8f9)."""

    def test_404_raises_not_found_error(self, extractor, mock_response):
        ...
        with patch.object(extractor, "_get_domain_session") as mock_session:
            ...
            with pytest.raises(NotFoundError, match="URL not found \\(404\\)"):
                extractor._extract_with_newspaper("https://example.com/missing")

    def test_410_raises_not_found_error(self, extractor, mock_response):
        """Test that 410 Gone response raises NotFoundError."""
        with patch.object(extractor, "_get_domain_session") as mock_session:
            mock_sess = Mock()
            mock_sess.get.return_value = mock_response(410, "Gone")
            mock_session.return_value = mock_sess

            with pytest.raises(NotFoundError, match="URL not found \\(410\\)"):
...
class TestFallbackBehavior:
    ...
    def test_parsing_error_allows_beautifulsoup_fallback(self, extractor):
        ...
        with (
            ...
            # Multiple calls: newspaper fails, check missing, check after BS, final check
...
class TestDeadURLCaching:
    ...
    def test_404_caches_url_as_dead(self, extractor, mock_response):
        ...
        with patch.object(extractor, "_get_domain_session") as mock_session:
            ...
            assert extractor.dead_urls[url] > 0  # Has expiration timestamp
...
class TestMetricsTracking:
    ...
    def test_404_tracks_metrics(self, extractor, mock_response):
        ...
        with patch.object(extractor, "_get_domain_session") as mock_session:
            ...
            # Check that end_method was called with False (failure)
