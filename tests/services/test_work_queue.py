...
def test_domain_assignment_no_overlap(coordinator):
    ...
    domains_data = [
        (f"domain{i}.com", f"Domain {i}", 10) for i in range(20)
    ]
    ...
    available_domains = [{"source": f"domain{i}.com", "article_count": 10} for i in range(20)]
    
    worker1_domains = coordinator._assign_domains_to_worker("worker-1", available_domains)
    coordinator.worker_domains["worker-1"] = {
        "domains": worker1_domains,
        "last_seen": time.time(),
    }
    
    worker2_domains = coordinator._assign_domains_to_worker("worker-2", available_domains)
    coordinator.worker_domains["worker-2"] = {
        "domains": worker2_domains,
        "last_seen": time.time(),
    }
    
    worker3_domains = coordinator._assign_domains_to_worker("worker-3", available_domains)
