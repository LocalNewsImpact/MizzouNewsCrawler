...
| article `curated_out` | absent from the human-curated March sheet, and therefore removed by that curation — for a reason the sheet does not record: wire, column, junk, non-local, anything. 3,627 rows. Terminal, not exported, and NOT a scope judgement: the August reconciliation filed these as `out_of_scope` and the funnel reported them as "Non-local" until 2026-09-13. `out_of_scope` now means only what a person or the scope step actually ruled non-local (74 rows). The `article_enrichment.skip_reason` `removed_in_march_review` keeps the provenance. | March reconciliation; funnel maps it to "Removed in March review" |
