...
class ExactContentCleaner:
    ...
    def analyze_domain(self, domain: str, sample_size: int = None, min_occurrences: int = 3) -> Dict:
        """
        ...
        if len(articles) < min_occurrences:
            self.logger.warning(f"Not enough articles ({len(articles)}) for domain {domain}")
    ...
    def _get_articles_for_domain(self, domain: str, sample_size: int = None) -> List[Dict]:
    ...
    def _find_exact_duplicate_segments(self, articles: List[Dict], min_occurrences: int) -> List[Dict]:
        """
        ...
        all_exact_matches = defaultdict(lambda: {
            ...
            "length": 0,
            "article_ids": set(),
            "positions": defaultdict(list),  # article_id -> [(start, end), ...]
        ...
        for match_key, match_info in all_exact_matches.items():
            if len(match_info["article_ids"]) >= min_occurrences:
                # Calculate position consistency (how often it appears at similar positions)
    ...
    def _find_exact_matches_between_articles(self, article1: Dict, article2: Dict) -> List[Tuple[str, List[Tuple[int, int]], List[Tuple[int, int]]]]:
        """
        ...
        for min_length in [100, 75, 50, 30]:  # Minimum lengths to consider
            ...
            for i in range(len(content1) - min_length + 1):
                ...
                for j in range(i + min_length, min(i + 500, len(content1) + 1)):  # Max 500 chars
    ...
    def _remove_overlapping_matches(self, matches: List[Tuple[str, List[Tuple[int, int]], List[Tuple[int, int]]]]) -> List[Tuple[str, List[Tuple[int, int]], List[Tuple[int, int]]]]:
        """Remove overlapping matches, keeping the longest ones."""
        if not matches:
            return matches
    ...
    def _calculate_position_consistency(self, positions: Dict[str, List[Tuple[int, int]]], articles: List[Dict]) -> float:
        """
        ...
        variance = sum((pos - mean_pos) ** 2 for pos in relative_positions) / len(relative_positions)
    ...
    def _classify_pattern(self, text: str) -> str:
        ...
        # Navigation patterns
        nav_keywords = ['news', 'sports', 'obituaries', 'contact', 'subscribe', 'home', 
        ...
        sub_keywords = ['subscribe', 'subscription', 'paywall', 'premium', 'members only']
    ...
    def _calculate_domain_stats(self, articles: List[Dict], segments: List[Dict]) -> Dict:
        ...
        return {
            ...
            "removal_percentage": (total_removable_chars / total_content_chars * 100) if total_content_chars > 0 else 0
        }
    
    def clean_article_content(self, content: str, segments_to_remove: List[Dict]) -> str:
        """Remove exact duplicate segments from article content."""
        cleaned_content = content
        
        # Sort segments by length (longest first) to avoid partial removals
        segments_sorted = sorted(segments_to_remove, key=lambda x: x["length"], reverse=True)
        ...
        cleaned_content = re.sub(r'^\s+|\s+$', '', cleaned_content)  # Leading/trailing whitespace
