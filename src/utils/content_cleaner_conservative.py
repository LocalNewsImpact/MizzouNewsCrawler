...
class ConservativeContentCleaner(ImprovedContentCleaner):
    ...
    def _apply_conservative_penalties(
        self, text: str, base_confidence: float, pattern_type: str
    ) -> float:
        ...
        journalism_count = sum(1 for term in journalism_terms if term in text_lower)
        ...
        news_pattern_matches = sum(
            1 for pattern in news_patterns if re.search(pattern, text_lower)
        )
        ...
        strong_boilerplate_count = sum(
            1 for term in strong_boilerplate_terms if term in text_lower
        )
    ...
    def _should_skip_segment(self, text: str, pattern_type: str) -> bool:
        ...
        if len(text) < 200 and not any(
            term in text_lower for term in ["subscribe", "login", "contact", "privacy"]
        ):
...
def create_conservative_cleaner(
    db_path: str = "data/mitzou.db",
) -> ConservativeContentCleaner:
