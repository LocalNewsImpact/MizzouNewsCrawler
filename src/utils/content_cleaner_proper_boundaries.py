...
class ProperBoundaryContentCleaner:
    ...
    def analyze_domain(
        ...
        duplicate_segments = self._find_duplicate_segments(
            proper_segments, min_occurrences
        )
    ...
    def _extract_complete_paragraphs(self, content: str) -> list[str]:
        ...
        for paragraph in paragraphs:
            ...
            if paragraph:
                ...
                if paragraph[-1] in ".!?:" or self._looks_like_heading(paragraph):
    ...
    def _find_duplicate_segments(
        self, proper_segments: dict[str, set[str]], min_occurrences: int
    ) -> list[dict]:
            ...
            if len(article_ids) >= min_occurrences:
                for article_id in article_ids:
                    if article_id not in articles_by_id:
                        cursor.execute(
                            "SELECT raw FROM articles WHERE id = ?", (article_id,)
                        )
                        row = cursor.fetchone()
                        if row:
