...
class FastExactContentCleaner:
    ...
    def analyze_domain(
        ...
        duplicate_blocks = self._find_duplicate_blocks(all_blocks, min_occurrences)
        ...
        for article in articles:
            ...
            for block_text, start_pos, end_pos in blocks:
                ...
                if len(normalized_text) >= 30:
                    all_blocks[normalized_text][article_id].append((start_pos, end_pos))

        return all_blocks

    def _find_duplicate_blocks(
        self, all_blocks: dict, min_occurrences: int
    ) -> list[dict]:
        """Find blocks that appear in multiple articles."""
        duplicate_blocks = []
    ...
    def _calculate_position_consistency(
        self, block_text: str, article_positions: dict
    ) -> float:
        ...
        for article_id, positions in article_positions.items():
            cursor.execute(
                "SELECT LENGTH(content) FROM articles WHERE id = ?", (article_id,)
            )
